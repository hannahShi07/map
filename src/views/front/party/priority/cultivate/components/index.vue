...
<script>
    ...
    import {getCultivateList,getCultivateMap} from "@/api/front/party/priority/cultivate/cultivate.js"
    ...
    export default {
        ...
        methods: {
            ...
            getStatistics(){
                ...
                let params = {
                        orgCode:this.orgId,
                        type:{key:'2279477819906842955'}
                }
                getCultivateMap(params).then(response => {
                    const res = response;
                    for(var i=1;i<res.length;i++){
                        this.yearList.push(res[i].year)//年份数组
                        this.awardedYearSum.push(res[i].awardedNumber)//组织增量数组
                        this.awardedSum.push(res[i].awardedYearSum)//	组织数数组
                    }
                    this.getEchart();//培育组织增量图表
                    this.getTissueyEchart();//培育组织数图表
