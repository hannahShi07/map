<template>
  <div>
    ...
    <div class="pd-20 mt-20">
      <el-table border :data="tableData" header-row-class-name="page-table-header" v-loading="loading"
                @row-click="editBtn">
        <el-table-column type="index" label="序号" width="60" align="center"/>
        <el-table-column label="学习专题" min-width="200px" align="left" show-overflow-tooltip>
          <template slot-scope="{row}">
            <span>{{row.themeName | tableEmptyText}}</span>
          </template>
        </el-table-column>

        <el-table-column label="学习状态" width="120px" align="center" show-overflow-tooltip>
          <template slot-scope="{row}">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 调用数据接口
      pageChange () {
        this.getEducationSpecialTotalFn()
        this.getEducationSpecialByPageFn()
      },
      // 获取表格数据
      getEducationSpecialByPageFn () {
        this.loading = true
        getEducationSpecialByPage({
          model: {
            year: this.year,
