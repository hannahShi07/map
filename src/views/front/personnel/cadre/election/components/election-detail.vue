<template>
  <div class="party" v-cloak>
    ...
    <div class="artContent pd-20">
      <h2 class="title">{{isEmpty(list,'changeTermTitle')}}</h2>
      <div class="contentDiv">
        <div class="dateRouter">
          <span>发布日期： {{isEmpty(list,'changeTermDate')}}</span>
          <span>发布单位： {{isEmpty(list,'changeTermOrgName')}}</span>
          <br>
<!--          <span>发布组织 ： {{list.articleSource}}</span>-->
          <span>参与人员: {{isEmpty(nameList)}}</span>
        </div>
        <div class="dateRouter">
          <span>换届地点：{{isEmpty(list,'changeTermAddress')}}</span>
        </div>
      </div>
      <div class="core" v-if="list.changeTermContent!='' && list.changeTermContent!=undefined && list.changeTermContent!=null && list.changeTermContent!='null'" v-html="list.changeTermContent">
...
<script>
  ...
  import {getElectionDetail} from "@/api/front/personnel/cadre/election/election.js"
  export default {
    ...
    methods:{
      ...
      getDetail(){
        ...
        getElectionDetail(params).then(response => {
          const res = response;
          this.list=res;
            this.list.memberName = ''
            let arr=[];
            for(let i=0;i<res.list.length;i++){
                    arr.push(res.list[i].name);
                    if(arr.length>10){
                        arr=arr.slice(0,10);
