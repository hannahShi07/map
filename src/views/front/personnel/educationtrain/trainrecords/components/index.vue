    ...
    <div class="contain" v-loading="loading">
      <div class="containlist" v-if="tableData!='' && tableData!=undefined && tableData!=null && tableData!='null' && tableData.length>0">
        <div class="list" @click="open(item)" v-for="(item,index) in tableData" :class="((index+1)%5)==0 ? 'borderBottom':''">
          <i></i>
          <div class="listTxt">{{item.theme}}</div>
          <span class="listTime">{{item.createTime}}</span>
...
<script>
  ...
  import {getTrainList} from "@/api/front/personnel/educationtrain/trainrecords/trainrecords.js"
  ...
  export default {
    ...
    methods: {
      ...
      //打开详情页
      open(row){
        this.$emit('handleSwitch', {name: 'RecordsDetail', params: row})
      },
    }
  }
</script>
