    ...
    <div class="contain" v-loading="loading">
      <div class="containlist" v-if="tableData!='' && tableData!=undefined && tableData!=null && tableData!='null' && tableData.length>0">
        <div class="list" @click="open(item)" v-for="(item,index) in tableData" :class="((index+1)%5)==0 ? 'borderBottom':''">
          <i></i>
          <div class="listTxt">{{item.title}}</div>
          <span class="listTime">{{item.publishDate}}</span>
...
<script>
  ...
  import {getRecruitList} from "@/api/front/personnel/talentrecruitment/recruitinfomanage/recruitinfomanage.js"
  export default {
    name: "index",
    components: {
    ...
    methods: {
      ...
      //打开详情页
      open(row){
        this.$emit('handleSwitch', {name: 'JobDetail', params: row})
      },
    }
  }
</script>
