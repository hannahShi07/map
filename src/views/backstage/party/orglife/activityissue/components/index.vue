...
<!--    <base-page-tree ref="refTree" @handleTreeClick="handleTreeClick">-->
      ...
      <div class="page-search-box">
        <base-label-box label="活动名称">
          <el-input v-model="name" @keyup.enter.native="editBtn(4)" placeholder="请输入会议名称" style="width: 280px" clearable/>
        </base-label-box>

        <base-label-box label="活动状态">
          <el-select
            v-model="status"
            value-key="id"
            filterable
        ...
        <base-label-box label="开始时间" class="beginTime">
          <el-date-picker
            v-model="beginTime"
            style="width: 280px"
            type="datetime"
...
<!--        <base-label-box label="结束时间" class="meetingTime">-->
<!--          <el-date-picker-->
<!--            v-model="endTime"-->
<!--            style="width: 280px"-->
<!--            type="datetime"-->
...
<!--        </base-label-box>-->
      ...
      <div class="pd-20 clearfix">
        <div class="fl">
          <base-page-button title="新增活动" imgType="add" type="primary" @click="editBtn(1)"/>
...
<!--          </el-table-column>-->
          ...
          <el-table-column label="活动名称" min-width="180px" align="left" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'name')}}</span>
            </template>
          </el-table-column>

<!--          <el-table-column label="活动类型" min-width="200px" align="left" show-overflow-tooltip>-->
<!--            <template slot-scope="scope">-->
<!--              <span>{{isEmpty(tableData[scope.$index],'orgName')}}</span>-->
<!--            </template>-->
<!--          </el-table-column>-->
          <el-table-column label="活动类型" width="140px" align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'type')}}</span>
            </template>
          </el-table-column>
          <el-table-column label="活动状态" width="140px" align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'status.data')}}</span>
            </template>
          </el-table-column>
          <el-table-column label="活动开始时间" width="160px" align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'beginTime')}}</span>
            </template>
          </el-table-column>
          <el-table-column label="活动地点" width="140px" align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'place')}}</span>
            </template>
          </el-table-column>

          <el-table-column label="查看报名表" width="140px" align="center" show-overflow-tooltip>
            <template slot-scope="{row}">
              <span class="isblue" @click="editBtn(3,row)">导出报名表</span>
            </template>
          </el-table-column>

          <el-table-column label="操作" width="300px" align="center" show-overflow-tooltip>
            <template slot-scope="{row}">
              <!--表格操作按钮组件-->
<!--              <base-table-a type="summary" @click="editBtn(6,row)" :class="{'color-grad':(userOrgcode!=row.orgCode || row.state.data=='未开始'  || row.state.data=='未发布')}">纪要</base-table-a>-->
              <base-table-a @click="editBtn(8,row)">查看</base-table-a>
              <base-table-a type="edit" @click="editBtn(7,row)" :class="{'color-grad':(row.status.data=='进行中'  || row.status.data=='已结束')}">编辑</base-table-a>
              <base-table-a type="delete" @click="editBtn(9,row)"  :class="{'color-grad':(row.status.data=='进行中'  || row.status.data=='已取消')}">删除</base-table-a>
            </template>
          </el-table-column>
...
<!--    <meeting-summary ref="refSummary" @refresh="pageChange"/>-->
    <!--详情-->
    <activityissue-detail ref="refDetail" @refresh=""/>
...
<script>
  ...
  import ActivityissueDetail from "./activityissue-detail"
  import db from '@/utils/localstorage'//  获取用户登录id
  import commonReq  from "@/api/public/commonReq.js"
  import {getMeetingList,getMeetingDetele,getMeetingExport} from "@/api/backstage/party/orglife/activityissue/activityissue.js"
  ...
  export default {
    ...
    methods: {
      ...
      getDict(){
        let params = {
          types:['activity_state']
        }
        commonReq.getHistoryDict(params).then(response =>{
          const res = response;
          this.dicts.life_state = res.activity_state;
      ...
      pageChange () {
        ...
        getMeetingList(params).then(response => {
          ...
          this.tableData.forEach(item=>{
            if(item.beginTime!='' && item.beginTime!=undefined && item.beginTime!=null && item.beginTime!='null'){
              item.beginTime=item.beginTime.substring(0,16)
            }
            if(item.endTime!='' && item.endTime!=undefined && item.endTime!=null && item.endTime!='null'){
      ...
      editBtn (t, row) {
        switch (t) {
          case 1:
          case 7:
            this.$emit('handleSwitch', {name: 'ActivityissueAdd', params: row})
            break
          ...
          case 9://删除
            row.isDelete='1'
            if(row.status!='' && row.status!=undefined && row.status!=null && row.status!='null'){
              row.status = row.status.key
            }
            if(row.message!='' && row.message!=undefined && row.message!=null && row.message!='null'){
              row.message = row.message.key
            }
            this.$confirm('是否删除该条信息？', '提示', {
              ...
              getMeetingDetele(row).then(response => {
