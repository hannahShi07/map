        ...
        >
          <el-table-column align="center" width="55" label="" >
            <template slot-scope="scope">
              <el-radio :label="scope.row.memberId" v-model="radioId">&nbsp</el-radio>
            </template>
          </el-table-column>
          ...
          <el-table-column label="党员姓名" min-width="120px" align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(selectData[scope.$index],'memberName')}}</span>
            </template>
          </el-table-column>
...
<script>
  import {getPartyMember} from "@/api/backstage/party/partymember/demonstration/demonstration.js"
  export default {
    name: "select-member",
    data(){
      return{
    ...
    methods:{
      ...
      selectPersons(list,id){
        ...
        this.loading = true
        getPartyMember(params).then(response => {
          const res = response;
          // console.log(res)
          this.tableData=res;
      ...
      selectName(){
        ...
        for(let i=0;i<this.tableData.length;i++){
          if(_this.tableData[i].memberName){
            if(_this.tableData[i].memberName.indexOf(_this.name) != -1){
