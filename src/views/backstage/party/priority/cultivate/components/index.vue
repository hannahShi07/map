<template>
  <div class="party">
      ...
      <base-org-import ref="refImport" @refresh="pageChange" :title="title" :action="action"
                         :templateUrl='templateUrl'  :otherData="otherData" :templateName='templateName' :isHaveOrg="false"/>
      <cultivate-add ref="refEdit" @refresh="pageChange"/>
      <cultivate-detail ref="refDetail" @refresh="pageChange"/>
...
<script>
    ...
    import cultivateAdd from './cultivate-add'
    import cultivateDetail from './cultivate-detail'
    import {getCultivateList,getCultivateExport,getCultivateDelete} from "@/api/backstage/party/priority/cultivate/cultivate.js"
    export default {
        name: "index",
        components: {cultivateAdd,cultivateDetail},
        data() {
          return {
            ...
            templateUrl:`${process.env.VUE_APP_IP_URL_PORT}/file/template/import/pykdr.xlsx`,
            templateName:"培育库导入模板.xlsx",
            title:'培育库导入',
              otherData:{//额外参数
                  headRows:'1',
                  titleRows:'2',
                  type:'2279477819906842955'
              },
          }
        },
        mounted() {
        ...
        methods: {
          ...
          getDict(){
            ...
            commonReq.getHistoryDict(params).then(response =>{
              const res = response;
              this.dicts.prize =res.prize.slice(4);
            })
          },
          //分页调取接口数据
          pageChange () {
            ...
            this.loading = true;
              getCultivateList(params).then(response => {
              const res = response;
            ...
            switch (t) {
              ...
              case 8:
                  this.$confirm('是否删除该条信息？', '提示', {
                      ...
                      getCultivateDelete(params).then(response => {
