  ...
  <div class="declare-detail" v-cloak v-loading="loading">
    <div class="title-bar clearfix">
      <div class="logotxt fl">
        <div class="logo-staybutton"></div><span  class="logo-staytext">{{baseList.checkState.data}}</span>
        <span>{{isEmpty(baseList,'orgId.data')}} {{isEmpty(baseList,'declareAwards.data')}}候选对象申请表</span>
<!--        <div v-if="baseList.checkState.data == '不通过'"><span>整改意见</span>-->
<!--          <span>{{baseList.checkOpinion}}</span>-->
<!--        </div>-->
      </div>
      ...
      <el-row class="card-row">
        <el-col :span="8">
          <div class="card-box">
            ...
            <div class="card-box-content">
              {{isEmpty(baseList,'preYearRevenue')}}
            </div>
          </div>
        </el-col>
        <el-col :span="16">
          <div class="card-box">
            ...
            <div class="card-box-content">
              {{isEmpty(baseList,'preYearRatepay')}}
            </div>
          </div>
        </el-col>
      </el-row>
      ...
      <el-row class="card-row">
        ...
        <el-col :span="8">
          <div class="card-box">
            ...
            <div class="card-box-content">
              {{isEmpty(baseList,'workExpenditure')}}
            </div>
          </div>
        </el-col>
      </el-row>
      ...
      <el-row class="card-row">
        <el-col :span="8">
          <div class="card-box">
            ...
            <div class="card-box-content">
              {{isEmpty(baseList,'developMemberNum')}}
            </div>
          </div>
        </el-col>
        <el-col :span="16">
          <div class="card-box">
            ...
            <div class="card-box-content">
              {{isEmpty(baseList,'partyActivelyNum')}}
            </div>
          </div>
        </el-col>
      </el-row>
...
<!--    </div>-->
    <div class="content">
      <base-card-box header="" :margin="{top: true}" style="width: 100%">
        ...
        <div class="card-title" v-if="results == '不通过'">
          <el-form ref="refForm" :model="formData" :rules="node">
            <el-form-item class="is-required" label="整改意见" prop="opinions">
              <div class="card-box">
                <el-input
                  placeholder="请输入"
    ...
    <div class="tac el-col el-col-24">
      <el-button @click="goBtn" class="page-btn page-btn-hollow">取消</el-button>
      <el-button @click="toSave" class="page-btn page-btn-hollow">确定</el-button>
    </div>
  </div>
</template>
...
<script>
    ...
    import {getDeclareDetail,getDeclareSave,getDeclareExport} from "@/api/backstage/party/priority/declare/declare.js"
    export default {
        name: 'declareAdd',
        props: {
            params: {
        ...
        methods: {
            ...
            //提交新项目
            addNewProject(){
                this.$emit('handleSwitch', {name: 'addDeclareproject'})
            },
            //编辑
            toEdit(row){
            ...
            toSave(){
                if(this.results == '通过'){
                    this.$confirm('确认通过'+this.baseList.orgId.data+this.baseList. declareAwards.data +'候选对象申请表？',
                        '确认通过', {
                    ...
                    this.$refs['refForm'].validate((valid) => {
                        if (!valid) {
                            ...
                            let params = {
                                checkOpinion: this.formData.opinions,
                                id: this.baseList.id,
            ...
            getDetail(){
                ...
                getDeclareDetail(params).then(respone => {
                    const res = respone
                    this.baseList = res
