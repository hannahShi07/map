  ...
  <div class="party">
    <base-page-tree  ref="refTree"   @handleTreeClick="handleTreeClick">
      ...
      <div class="page-search-box">
        ...
        <!--          <el-input v-model="orgName" placeholder="请输入" style="width: 280px" clearable-->
      ...
      <div class="mr-20 ml-20" v-cloak>
        <el-table :data="tableData" tooltip-effect="dark" row-key="id" v-loading="loading"
          ...
          <el-table-column label="批准机关" align="center" show-overflow-tooltip>
            <template slot-scope="{row}">
        ...
        <base-pagination :total="total" :pageNum.sync="pageNum" :pageSize.sync="pageSize"
                         @pageChange="getAwardOrgByPageFn"/>
      </div>
    </base-page-tree>

    <!-- 导入 -->
    <base-org-import ref="refImport" @refresh="editBtn(1)" title="导入组织奖惩" :action="action" :templateUrl="templateUrl"
                     :otherData="otherData" :isHaveOrg="true" :fixed="true" :templateName="templateName"/>

    <!-- 新增修改 -->
    <org-edit-dialog ref="refEdit" @refresh="editBtn(1)"/>

    <!-- 详情 -->
    <org-msg-dialog ref="refMsg"/>
  </div>
</template>

<script>
  import OrgMsgDialog from './org-msg-dialog'
  import OrgEditDialog from './org-edit-dialog'
  import commonReq from '@/api/public/commonReq'
  import {getAwardOrgByPage, delAwardOrg, getAwardOrgExport} from '@/api/backstage/party/award/org'

  export default {
    components: {OrgEditDialog, OrgMsgDialog},
    data () {
      return {
        // 组织编码
        orgCode: '',
        ...
        pageSize: 10,
        // 导入接口
        action: `${process.env.VUE_APP_BASE_API}/partycloud/djOrgPrizePunish/importOrgPrizePunish`,
        // 导入模板
        templateUrl: `${process.env.VUE_APP_IP_URL_PORT}/file/template/import/zzjc.xlsx`,
        templateName: '组织奖惩模版.xlsx',
        otherData: {
          orgId: '1'
        }
      }
    ...
    mounted () {
      this.getCondition()
      this.getAwardOrgByPageFn(true)
    },
    methods: {
      //字典表数据
      getCondition () {
        commonReq.getHistoryDict({types: ['org_rp_type']}).then(response => {
          const res = response
          this.typeList = res.org_rp_type
        })
      },
      //获取列表数据
      getAwardOrgByPageFn (refresh) {
        this.loading = true
        if (refresh === true) {
          this.date = ''
          this.orgName = ''
        ...
        switch (t) {
          ...
          case 8:
            this.$confirm('是否删除该条信息？', '提示', {
              ...
              delAwardOrg({ids: [row.id]}).then(response => {
