<template>
  <div class="return">
    ...
    <div class="page-search-box">
      <base-label-box label="单位">
        <el-input placeholder="请输入" v-model="orgName" clearable @keyup.enter.native="checkHandle()"/>
      </base-label-box>
      <base-label-box label="服务类型">
        <el-select v-model="type.key" placeholder="请选择" :editable="false" clearable @change="checkHandle()">
          <el-option
    ...
    <div class="mr-20 ml-20" v-cloak>
      ...
      >
        ...
        <el-table-column label="单位" width="160px" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{isEmpty(tableData[scope.$index],'orgName')}}</span>
          </template>
        </el-table-column>

        <el-table-column label="服务职能" min-width="180px" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{isEmpty(tableData[scope.$index],'serviceFunction')}}</span>
          </template>
        </el-table-column>

        <el-table-column label="服务类型" min-width="160px" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
    ...
    <!-- 新增/编辑 -->
    <servicework-add ref="refEdit" @refresh="pageChange" />
    <servicework-detail ref="refInfo" @refresh="pageChange"/>
  </div>
</template>

<script>
import commonReq  from "@/api/public/commonReq.js"
import serviceworkAdd from './servicework-add'
import serviceworkDetail from './servicework-detail'
import {getpageList,getpageDelete} from "@/api/backstage/sysset/bigscreen/alliance/servicework/servicework.js"
export default {
  components: {
    serviceworkAdd,
    serviceworkDetail
