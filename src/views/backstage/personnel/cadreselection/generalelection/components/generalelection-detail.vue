      ...
      <div class="content-box">
        ...
        <div class="content-txt">
          <div class="left-0">
            ...
            <span>发布单位：{{isEmpty(detailCont,'changeTermOrgName')}}</span>
            <br>
            <span>参与人员：{{isEmpty(detailCont,'memberName')}}</span>
          </div>
          <div class="right-0"><span>换届地址：{{isEmpty(detailCont,'changeTermAddress')}}</span></div>
        </div>
        ...
        <!--</div>-->
        <div class="content-main" v-if="detailCont.changeTermContent" v-html="detailCont.changeTermContent"/>
        ...
        methods: {
            ...
            getDetail(){
                ...
                getGeneralelectionDetail(params).then(response => {
                    const res = response;
                    this.detailCont=res;
                    this.detailCont.memberName = ''
                    for (let i = 0;i<res.list.length;i++){
                        this.detailCont.memberName += res.list[i].name+'、'
                    }
                }).finally(() => {
                });
            },
