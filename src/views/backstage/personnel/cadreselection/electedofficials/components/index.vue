...
<template>
  <div class="party">
      ...
      <div class="page-search-box">
        ...
        <el-input v-model="djOfficerTitle" @keyup.enter.native="editBtn(2)" placeholder="请输入" style="width: 280px" clearable/>
      </base-label-box>
        <base-label-box label="发布单位" class="title1">
          <el-input v-model="djOfficerOrg" @keyup.enter.native="editBtn(2)" placeholder="请输入" style="width: 280px" clearable/>
        </base-label-box>
        <base-label-box label="发布日期" class="title1">
          <el-date-picker v-model="djOfficerDate"  @change="selectdate" type="date" placeholder="请选择" style="width: 280px" clearable value-format="yyyy-MM-dd"/>
        </base-label-box>
      </div>
      <!-- 按钮区 -->
      ...
      <div class="mr-20 ml-20" v-cloak>
        ...
        >
          ...
          <el-table-column label="信息标题" min-width="200px" align="left" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'djOfficerTitle')}}</span>
            </template>
          </el-table-column>

          <el-table-column label="发布单位" min-width="180px"  align="left" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'djOfficerOrg')}}</span>
            </template>
          </el-table-column>

          <el-table-column label="发布日期" width="140px" align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'djOfficerDate')}}</span>
            </template>
          </el-table-column>
    ...
    <!--新增-->
    <electedofficials-add ref="refAdd" @refresh="pageChange"/>
  </div>
</template>

<script>
    import ElectedofficialsAdd from "./electedofficials-add"
    // import commonReq  from "@/api/public/commonReq.js"
    import {getElectedofficialsList,getElectedofficialsDetele} from "@/api/backstage/personnel/cadreselection/electedofficials/electedofficials.js"
    export default {
        name: "index",
        components: {
            ElectedofficialsAdd
        ...
        methods: {
            ...
            pageChange () {
                let params = {
                    ...
                    model: {
                        djOfficerTitle:this.djOfficerTitle,
                        djOfficerDate:this.djOfficerDate,
                        djOfficerOrg:this.djOfficerOrg
                    },
                    size: this.pageSize
                }
                this.loading = true;
                getElectedofficialsList(params).then(response => {
                    const res = response;
                ...
                switch (t) {
                    ...
                    case 6://删除
                        this.$confirm('是否删除该条信息？', '提示', {
                            ...
                            getElectedofficialsDetele(params).then(response => {
