<template>
  ...
  >
    <el-form :model="formData" ref="refForm" label-width="90px" :loading='loading' v-if="visible">
      <div class="examInfo">
        <p style="font-size:24px;color:#555;padding:20px;">{{formData.examName}}</p>
        <div style="padding:0 20px 20px;border-bottom:1px solid #eee;">
          <span style="color:#ED3C3C;font-size:14px">此页面为考试信息预览，并非正式考试页面</span>
          <span style="float:right;">起止时间 {{formData.beginTime}} - {{formData.endTime}}</span>
        </div>
        ...
        <div style="padding:20px;padding-bottom:0;margin-bottom:1px;border-bottom:1px solid #eee;" v-if='formData.singleChoice.length'>
          ...
          <div class="titleContent" v-for="(item,index) in formData.singleChoice" :key="index" :style="hasBorder(formData.singleChoice.length-1,index)">
            ...
            <p style="font-size:12px;color:#FC7121;margin:10px 0;">&nbsp;&nbsp;&nbsp;&nbsp;正确答案: {{item.answer}}</p>
            <p v-for='(item,idx) in item.choose' :key='idx' style="margin-top:10px;">
              &nbsp;&nbsp;&nbsp;&nbsp;<span style="display:inline-block;width:16px;height:16px;border:1px solid #ccc;border-radius:50%;margin-right:10px;vertical-align: bottom;"></span>{{item}}
            </p>
          </div>
        </div>
        ...
        <div style="padding:20px;padding-bottom:0;margin-bottom:1px;border-bottom:1px solid #eee;" v-if='formData.multipleChoice.length'>
          ...
          <div class="titleContent" v-for="(item,index) in formData.multipleChoice" :key="index" :style="hasBorder(formData.multipleChoice.length-1,index)">
            ...
            <p style="font-size:12px;color:#FC7121;margin:10px 0;">&nbsp;&nbsp;&nbsp;&nbsp;正确答案: {{item.answer}}</p>
            <p v-for='(item,idx) in item.choose' :key='idx' style="margin-top:10px;">
              &nbsp;&nbsp;&nbsp;&nbsp;<span style="display:inline-block;width:16px;height:16px;border:1px solid #ccc;margin-right:10px;vertical-align: bottom;"></span>{{item}}
            </p>
          </div>
        </div>
        ...
        <div style="padding:20px;padding-bottom:0;margin-bottom:1px;border-bottom:1px solid #eee;" v-if='formData.fillBlank.length'>
          ...
          <div class="titleContent" v-for="(item,index) in formData.fillBlank" :key="index" :style="hasBorder(formData.fillBlank.length-1,index)">
            ...
            <p style="font-size:12px;color:#FC7121;margin:10px 0;">&nbsp;&nbsp;&nbsp;&nbsp;正确答案: {{item.answer.join(',')}}</p>
          </div>
        </div>
...
<script>
  ...
  import { previewPaper } from '@/api/backstage/education/exam'
