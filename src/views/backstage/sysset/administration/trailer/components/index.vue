    ...
    <div class="pd-20 clearfix">
      ...
      <div class="mr-20 ml-20" v-cloak>
        ...
        >
          ...
          <el-table-column label="宣传片名称" min-width="200px" align="left" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'name')}}</span>
            </template>
          </el-table-column>

          <el-table-column label="发布日期" width="120px" align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'pubdate')}}</span>
            </template>
          </el-table-column>

          <el-table-column label="浏览数" min-width="120px"  align="center" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{isEmpty(tableData[scope.$index],'pv')}}</span>
            </template>
          </el-table-column>
      ...
      <trailer-add ref="refEdit" @refresh="pageChange" />
      <trailer-info ref="refInfo" @refresh="pageChange" />
  </div>
</template>

<script>
import {gatewayList,gatewayDelete} from "@/api/backstage/sysset/administration/gateway/gateway.js"
import TrailerAdd from './trailer-add'
import TrailerInfo from './trailer-info'
export default {
  components: {TrailerAdd,TrailerInfo},
  data () {
    return {
      loading:false,
      tableData:[],
