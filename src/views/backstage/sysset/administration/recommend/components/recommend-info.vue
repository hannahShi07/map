    ...
    <el-dialog
      ...
      <el-form ref="refForm" :model="formData" :rules="node" label-width="140px">
        <div class="information">
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="information-content" label="项目概述" prop="">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="投资总额（万元）" prop="">
                {{isEmpty(formData.investment)}}
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="项目成熟度" prop="">
                {{isEmpty(formData.projectMaturity)}}
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12">
              <el-form-item label="行业分类" prop="">
                {{isEmpty(formData.industryCategory)}}
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="合作方式" prop="">
                {{isEmpty(formData.cooperationType)}}
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="项目载体" prop="">
                {{isEmpty(formData.carrier)}}
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
import {gatewayInfo,clickNum} from "@/api/backstage/sysset/administration/gateway/gateway.js"
export default {
  name: "DemeanorInfo",
  components: {
  ...
  methods: {
    ...
    getDetail(id){
      ...
      gatewayInfo(params).then(response => {
        response.carrier = response.carrier.data
        response.industryCategory = response.industryCategory.data
        response.cooperationType = response.cooperationType.data
        let projectArr = []
        if(response.projectMaturityArr){
          for(let i=0;i<this.projectMaturityList.length;i++){
            if(response.projectMaturityArr.indexOf(this.projectMaturityList[i].id)>-1){
              projectArr.push(this.projectMaturityList[i].name)
            }
          }
        }
        response.projectMaturity = projectArr.join(",")
