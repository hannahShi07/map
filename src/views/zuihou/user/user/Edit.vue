<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="相关附件" prop="fileLength">
        <fileUpload :acceptSize="2*1024*1024" :auto-upload="true" :limit="5" @fileLengthVaild="fileLengthVaild" @setId="setIdAndSubmitFile" ref="fileRef">
...
export default {
  name: "UserEdit",
  components: { Treeselect, imgUpload, fileUpload},
  props: {
    dialogVisible: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    async setUser(val, orgs, dicts, enums) {
        const vm = this;
        if (val) {
            vm.user = {...val};
        } else {
            await commonApi.generateId().then(response => {
                const res = response.data;
                if (res.isSuccess) {
                    vm.user.id = res.data;
                }
    ...
    // 设置业务id
    setIdAndSubmitFile (isUploadCompleted, bizId) {
        if (isUploadCompleted) {
            if(this.user.id === null || this.user.id === ''){
                this.user.id = bizId.data.bizId;
            }
        }
    }
  }
