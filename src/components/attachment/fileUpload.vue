...
export default {
  ...
  methods: {
    // 附件初始化
    init({ id, bizId, bizType, isSingle, isDetail, isUpload}) {
      const vm = this;
    ...
    handleChange(file, fileList) {
      ...
      if (file.response) {
        if (file.response.isSuccess) {
          ...
          vm.cnt += 1;
          vm.ids.push(file.response.data.id);
          if(vm.cnt === fileList.length){
      ...
      // vm.$emit("setId", true, file.response);
      } else {
        if (vm.acceptSize) {
    ...
    handleRemove(file) {
      const vm = this;
        if (file.response) {
            if (file.response.isSuccess) {
                vm.ids.splice(vm.ids.indexOf(file.response.data.id),1);
                vm.cnt -= 1;
            }
        }else{
            if (file.bizId) {
            ...
            vm.cnt -= 1;
            vm.ids.splice(vm.ids.indexOf(file.id),1);
        }
    ...
    async getAttachment() {
      const vm = this;
      if(vm.fileOtherData.bizId && vm.fileOtherData.bizType){
    ...
    submitFiles() {
        commonApi.submitFiles({
            bizId: this.fileOtherData.bizId,
            bizType: this.fileOtherData.bizType,
            ids: this.ids
        });
    }
